"use client"
import React, { Dispatch, SetStateAction } from 'react';
import { Toggle } from "@/component/ui/toggle";

const cardType = {
  FIRE: "火炎",
  ICE: "氷水",
  NATURE: "自然",
  DARKNESS: "暗黒",
  RARE: "珍種",
  OPTION: "ＯＰ"
}

const maxSelect = 2

function Card({ card, cardState }: { card: Card, cardState: { card: number[], setCard: Dispatch<SetStateAction<number[]>> } }) {
  function handleCardChange() {
    if (cardState.card.includes(card.number)) {
      cardState.setCard(cardState.card.filter(_ => _ != card.number))
    }
    else if (cardState.card.length < maxSelect) {
      cardState.setCard([...cardState.card, card.number])
    }
  }

  return (
    <Toggle
      data-slot="card"
      className={
        "data-[state=on]:text-white data-[state=on]:bg-transparent data-[state=on]:hover:bg-transparent " +
        "p-0 relative size-[100px] text-white hover:text-white hover:bg-transparent bg-transparent text-center rounded-none border-6 " +
        (card.type == cardType.FIRE ? "border-game-fire" :
          card.type == cardType.ICE ? "border-game-ice" :
            card.type == cardType.NATURE ? "border-game-nature" :
              card.type == cardType.DARKNESS ? "border-game-darkness" :
                card.type == cardType.RARE ? "border-game-rare" :
                  "border-game-option")
      }
      onPressedChange={handleCardChange}
    >
      <img className=
        {cardState.card.includes(card.number) ?
          "opacity-100 hover:opacity-25" :
          "opacity-25 " + (cardState.card.length < maxSelect ? "hover:opacity-100" : "")}
        src={`/asset/image/card/${card.img}.png`} alt=" " />
      {!!card.lv && <div className="absolute top-0 left-0 w-2/5 text-left ml-1">{card.lv}</div>}
      <div className="absolute bottom-0 right-0 w-2/5 text-right mr-1">{card.number.toString().padStart(3, "0")}</div>
    </Toggle>
  );
}

export { Card }
